...
import {
  ...
  RefreshCw, Headphones, Tag, Package, type LucideIcon,
} from "lucide-react";
...
function CatBtn({ c }: { c: HomeCategory }) {
  ...
  return (
    <Link href={c.href} className="group flex flex-col items-center gap-1.5">
      <div className="flex h-[72px] w-[72px] items-center justify-center rounded-full border border-gray-200 bg-white p-4 text-gray-500 transition group-hover:border-blue-500 group-hover:bg-blue-50 group-hover:text-blue-600 group-hover:shadow-sm">
        {icon ?? (
          <ImageWithFallback
            src={c.image}
            alt={c.name}
      ...
      <span className="text-center text-[11px] leading-tight text-gray-700 group-hover:text-blue-600">
        {c.name}
      </span>
    </Link>
  );
...
const BENEFITS: { icon: LucideIcon; label: string }[] = [
  { icon: Shield,    label: "36 Months EMI" },
...
const WHY = [
  {
    icon: ShoppingCart, accent: "border-l-yellow-400",
  ...
  {
    icon: Shield, accent: "border-l-purple-500",
  ...
  {
    icon: Headphones, accent: "border-l-green-500",
...
export default function Home() {
  ...
  const aS = S("apple-shopping-event");
  ...
  return (
    <>
      ...
      <section className="bg-white pt-2 pb-2">
        <div className={W}>
          <div className="grid gap-2 lg:grid-cols-3">
            ...
            >
              <div className="relative h-[420px] overflow-hidden bg-gray-100 sm:h-[500px]">
                {bannerData[0]?.image ? (
                  <img
                  ...
                  <div className="absolute inset-0 flex flex-col items-start justify-center bg-gradient-to-r from-gray-100 to-blue-50 px-10">
                    <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400">Authentic. Reliable.</p>
                    <h2 className="mt-2 text-4xl font-black text-gray-900 leading-tight">Unseen<br />Gadget</h2>
                    <p className="mt-1 text-xs text-gray-400">www.unseengadget.com</p>
                    <span className="mt-5 inline-flex items-center gap-1.5 rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white">
                      Go Shopping <ChevronRight className="h-4 w-4" />
                    </span>
                  </div>
                )}
              </div>
            ...
            <div className="flex flex-col gap-2">
              {bannerData.slice(1, 3).map((b) => (
                ...
                >
                  <div className="relative h-full min-h-[148px] overflow-hidden bg-blue-50">
                    {b.image ? (
                      <img
                        src={b.image}
                        alt={b.title}
                      ...
                      <div className="absolute inset-0 flex flex-col justify-center px-5">
                        <p className="text-base font-black text-gray-900">{b.title}</p>
                        {b.subtitle && (
                          <p className="mt-0.5 text-xs text-gray-500">{b.subtitle}</p>
                        )}
                      </div>
                    )}
                  </div>
      ...
      <section className="border-y border-gray-100 bg-gray-50">
        <div className={W}>
          <div className="grid grid-cols-2 divide-x divide-gray-100 sm:grid-cols-3 lg:grid-cols-5">
            {BENEFITS.map((b) => (
              <div key={b.label} className="flex items-center justify-center gap-2 py-2.5">
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-blue-50 text-blue-600">
                  <b.icon className="h-3.5 w-3.5" />
                </span>
                <span className="text-[11.5px] font-medium text-gray-700">{b.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-white py-6">
        <div className={W}>
          <div className="mb-5 text-center">
            ...
            <h2 className="mt-1.5 text-[19px] font-bold text-gray-900">
              Browse through our top categories to find the products you&rsquo;ll love
            </h2>
          </div>
      ...
      <section className="border-t border-gray-100 bg-white py-5">
        <div className={W}>
          <SectionHeading title="iPads" href={iS?.viewAllHref ?? "/category/ipads-tablets"} />
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {iPs.map((p) => <ProductCard key={p.id} product={p} />)}
          </div>
        </div>
      </section>

      {/* ══ NEW ARRIVAL — left promo + right grid ═════════════ */}
      <section className="border-t border-gray-100 bg-white py-5">
        <div className={W}>
          <div className="grid gap-3 lg:grid-cols-5">
            {/* Left promo — Mac mini M4 */}
            <div className="hidden overflow-hidden rounded-xl bg-gradient-to-br from-blue-50 via-white to-blue-100 lg:flex lg:flex-col lg:items-center lg:justify-center">
              <div className="px-4 py-6 text-center">
                <p className="text-[9px] font-bold uppercase tracking-widest text-gray-400">New</p>
                <p className="mt-0.5 text-[32px] font-black leading-none text-gray-900">Mac&nbsp;mini</p>
                <p className="text-[28px] font-black leading-none text-blue-600">M4</p>
                <p className="mt-1.5 text-[11px] text-gray-500">Mighty power. Mini size.</p>
              </div>
            </div>

            <div className="lg:col-span-4">
              <SectionHeading title="New Arrival" href={nS?.viewAllHref ?? "/new-arrivals"} />
      ...
      <section className="border-t border-gray-100 bg-white py-5">
        <div className={W}>
          <SectionHeading title="Budget-Friendly Tablets!" href={bS?.viewAllHref ?? "/category/ipads-tablets"} />
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            {bPs.map((p) => <ProductCard key={p.id} product={p} />)}
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-gray-100 bg-white py-5">
        <div className={W}>
          <SectionHeading title="Top Selling" href={tS?.viewAllHref ?? "/top-selling"} />
      ...
      <section className="border-t border-gray-100 bg-white py-6">
        <div className={W}>
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-[18px] font-bold text-gray-900">Handpicked Products for You</h2>
            <Link
              href="/products"
      ...
      {/* ══ APPLE EVENT — dark section ═══════════════════════ */}
      {aS && aPs.length > 0 && (
        <section className="bg-[#111] py-5">
          <div className={W}>
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h2 className="text-[18px] font-bold text-white">{aS.title}</h2>
                {aS.subtitle && (
                  <p className="mt-0.5 text-[11px] text-gray-400">{aS.subtitle}</p>
                )}
              </div>
              <Link
                href={aS.href ?? "/category/computers"}
                className="flex items-center gap-1 rounded-full border border-gray-600 px-3 py-1.5 text-[11px] font-medium text-gray-300 transition:hover:border-white:hover:text-white"
              >
                {aS.cta ?? "Go Shopping"} <ChevronRight className="h-3 w-3" />
              </Link>
            </div>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-5">
              {aPs.slice(0, 5).map((p) => (
                <Link
                  key={p.id}
                  href={`/product/${p.slug}`}
                  className="group overflow-hidden rounded-xl border border-gray-700 bg-gray-800 transition:hover:border-gray-500"
                >
                  <div className="flex aspect-square items-center justify-center bg-white/5 p-3">
                    {p.image
                  ...
                  <div className="px-2.5 pb-3">
                    <p className="line-clamp-2 text-[11.5px] text-gray-300 group-hover:text-white">{p.name}</p>
                    <p className="mt-1 text-[13px] font-bold text-white">{formatBDT(p.price)}</p>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <section className="border-t border-gray-100 bg-white py-6">
        <div className={W}>
          ...
          <h2 className="mt-1.5 max-w-xl text-[20px] font-bold leading-snug text-gray-900">
            Bangladesh's Trusted Online Store for Genuine Tech & Accessories
          </h2>
          <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {WHY.map((card) => (
              <div
                key={card.title}
                className={`rounded-xl border border-gray-100 bg-white p-5 shadow-sm border-l-4 ${card.accent}`}
              >
                <card.icon className="h-6 w-6 text-gray-400" strokeWidth={1.5} />
                <h3 className="mt-3 text-[13px] font-bold text-gray-900">{card.title}</h3>
                <p className="mt-1.5 text-[12px] leading-relaxed text-gray-600">{card.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="border-t border-gray-100 bg-white py-6">
        <div className={W}>
          <h2 className="mb-4 text-[18px] font-bold text-gray-900">Our Articles</h2>
            ...
            {articleData.slice(0, 4).map((a) => (
              ...
              >
                <div className="aspect-[16/9] overflow-hidden bg-gray-100">
                  {a.image ? (
                    ...
                    <div className="flex h-full items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100">
                      <span className="text-[11px] font-medium text-blue-400">{a.category}</span>
                    </div>
                  )}
                </div>
                <div className="flex flex-1 flex-col p-3">
                  ...
                  <h3 className="mt-1.5 line-clamp-2 text-[12.5px] font-semibold text-gray-800 group-hover:text-blue-600">
                    {a.title}
                  </h3>
                  <p className="mt-1 line-clamp-3 text-[11.5px] leading-relaxed text-gray-500">
                    {a.excerpt}
                  </p>
                  <span className="mt-auto pt-2.5 text-[11.5px] font-semibold text-blue-600 group-hover:underline">
                    Continue Reading
                  </span>
                </div>
              </Link>
      ...
      <section className="border-t border-gray-100 bg-white py-6">
        <div className={W}>
          <h2 className="text-[15px] font-bold text-gray-900">Welcome to Unseen Gadget</h2>
          <p className="mt-2 max-w-3xl text-[12.5px] leading-relaxed text-gray-600">
            ...
            we're driven by big dreams and a commitment to provide you with a diverse range of the latest
          ...
          <button className="mt-3 flex items-center gap-1.5 rounded-xl border border-gray-300 px-3.5 py-1.5 text-[11.5px] font-medium text-gray-600 transition hover:border-gray-400">
            Read More <ChevronRight className="h-3 w-3" />
          </button>

          <h2 className="mt-7 text-[15px] font-bold text-gray-900">
          ...
          <p className="mt-2 max-w-3xl text-[12.5px] leading-relaxed text-gray-600">
            ...
            At Unseen Gadget — Bangladesh's Leading online Gadget Shop — we pride ourselves on offering genuine
