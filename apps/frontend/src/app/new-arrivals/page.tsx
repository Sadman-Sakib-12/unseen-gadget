...
import { useNewArrivals } from "@/hooks/use-queries";

export default function NewArrivalsPage() {
  const { t } = useTranslation();
  const { data: newArrivalsRes, isLoading } = useNewArrivals();

  const newArrivals: MockProduct[] = useMemo(() => {
    const raw = newArrivalsRes as any;
      ...
      <div className="container-gadget py-8">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border bg-card py-20 text-center">
            ...
            <Link href="/" className="btn-primary mt-4 rounded-xl">
